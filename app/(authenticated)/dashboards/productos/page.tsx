...
export default function ProductosPage(): JSX.Element {
  ...
  function focusTable(estado: string): void {
    setTableEstado(estado);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-xl font-bold text-text-primary">Productos e inventario</h1>
          <p className="text-sm text-text-muted">Qué productos importan, cómo se mueven y qué decisión tomar.</p>
        </div>
        <div className="flex gap-1">
          {WINDOWS.map((w) => (
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-xs font-medium ${window === w.value ? "bg-surface-dark text-text-inverse" : "bg-surface-alt text-text-secondary"}`}
            >
              {w.label}
            </button>
          ))}
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <DecisionCard
              ...
              accent="#C2410C"
              showValor
              onVerTodos={() => focusTable("sobrestock")}
            />
            <DecisionCard
          ...
          {/* Tabla principal */}
          <div ref={tableRef}>
            <ProductsTable key={`${window}-${tableEstado}`} window={window} initialEstado={tableEstado} />
          </div>
        </>
      ) : (
